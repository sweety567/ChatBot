...
chat_history_text = tk.Text(window, width=70, height=20)
...
def speech_input():
    with sr.Microphone() as source:
        # Update GUI with speech prompt
        chat_history_text.insert(tk.END, "User: Speak out your feelings...\n")
        window.update()
...
def speak_text(text):
    tts = gTTS(text)
    tts.save("C:/Users/saini/Downloads/response.mp3")

    # Initialize pygame mixer
    pygame.mixer.init()

    # Load the audio file
    pygame.mixer.music.load("C:/Users/saini/Downloads/response.mp3")
...
def therapy_chatbot():
    ...
    negation_list = ["not"]
    data = pd.read_csv("C:/Users/saini/Downloads/response_sheet.csv").values
    result = {}
    ...
    data = pd.read_csv("C:/Users/saini/Downloads/training.csv")
    description_list = []
    ...
    def process_input(user_input):
        ...
        # Display user input and chatbot response in the chat history text widget
        #chat_history_text.insert(tk.END, f"User: {user_input}\n")
        window.update()
        
        chat_history_text.insert(tk.END, f"Chatbot: {response}\n")
        chat_history_text.insert(tk.END, "-" * 50 + "\n")
